...
app.post("/search", async (req, res, next) => {
  try {
    ...
    const result = await axios.get(
      `${baseUrl}/search?key=${apiKey}&type=video&part=snippet&q=${searchQuery}`
    )
    ...
    if (temp != null) {
      await playListModel.findByIdAndUpdate(temp, {
        list: [
          ...temp.list,
          {
            name: result.data.items[0].snippet.title,
            year: result.data.items[0].snippet.publishTime,
          },
        ],
      })
      res
        .status(200)
        .json({ msg: ` ${result.data.items[0].snippet.title} is added!` })
    } else {
...
app.get("/findplaylist", async (req, res) => {
  ...
  if (result == null) res.status(400).send("playlist dosnt found")
  else {
    let newList = result.list.map((val) => {
      return val.name
    })
